...
In this take-home exercise, we will focus on **3, 4, 5 Room HDB in the study period of 2022.** Hence from the above, some consideration of the visualization analysis are:

-   Resale price by 2 different flat type. We choose standard and DBSS in this exercise.

-   Resale price by flat type, which is 3, 4 or 5 Room HDB.

-   Transaction volume by flat types

-   Resale price by town

-   Transaction volume by town

-   Resale price and floor area, with different colors or shapes representing different towns
...
From the above t-test, we see a general linear correlation between floor area (sqm) VS resale price.

However, the r value is not very high and there are some points fall far away from the linear line.

We suspect it might due to different location and flat type.

Hence now we will narrow down to the same location and flat type.

```{r}
geylang_data <- standard_data %>%
  filter(town == ("GEYLANG"), flat_type == ("3 ROOM") )
```

Then we build another Significant Test of Correlation between floor area (sqm) and resale price (S\$).
...
The correlation is even lower. So town location and flat type may not be the most important factor.

As there is no clear correlation found, we will continue the exploration by looking into Multiple regression model.
...
There are a few outliers for 3 Room flat resale prices.

The resale prices for 4 Room flat are closely distributed.
...
We use geom_tile() from ggplot() to plot a heatmap to visualize the correlation between different variables, such as resale price, floor area, and lease commence date/ year.
...
When doing a visual analysis of HDB resale price in Singapore, it is important to visualize the uncertainty of the estimates in addition to the estimates themselves. This is because the resale price of HDB flats is influenced by various factors, such as location, age of the flat, size of the flat, and various market conditions. These factors can lead to considerable uncertainty in the estimated resale price.

By visualizing the uncertainty, we can gain a better understanding of the range of possible values that the true resale price could take, given our data and assumptions.

One way to visualize uncertainty is by using error bars, which can show the range of values that the estimate is likely to fall within, given a certain level of confidence or probability.

Visualizing uncertainty also helps to identify outliers or extreme values that may have a significant impact on the estimates, as well as potential sources of bias or error in the data or methodology used to generate the estimates. It can also help to highlight areas where further research or data collection may be needed to improve the accuracy and reliability of the estimates.

In summary, visualizing uncertainty is an important part of any data analysis, as it helps to provide a more complete and accurate picture of the data and the estimates that are derived from it.
